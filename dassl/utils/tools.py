...
def read_image(path):
    """Read image from path using ``PIL.Image``, with error handling.

    Args:
        path (str): path to an image.

    Returns:
        PIL image if successful, or a 256x256 black image if an error occurs.
    """
    try:
        return Image.open(path).convert("RGB")
    except (FileNotFoundError, PIL.UnidentifiedImageError, IOError) as e:
        print(f"Error reading image at {path}: {e}")
        # Return a black image of size 256x256
